...
class DataGeneratorPicklesTrain(Sequence):
    ...
    def __init__(self, data_dir, filename, mini_batch_size, input_size, batch_size=10):
        ...
        self.x, self.yh, self.y, self.z, self.weights = self.prepareXYZ(
            data_dir, filename)
    ...
    def prepareXYZ(self, data_dir, filename):
        ...
        y = np.array(Z['y_true'][:1, :], dtype=np.float32)
...
class DataGeneratorCondPicklesTrain(Sequence):
    ...
    def __init__(self, data_dir, filename, mini_batch_size, input_size, batch_size=10):
        ...
        self.max_1 = (self.x.shape[0] // self.mini_batch_size) - 1
        self.max = (self.max_1 // self.batch_size) - 1
        self.training_steps = self.max
    ...
    def prepareXYZ(self, data_dir, filename):
        ...
        yh = np.array(Z['y_l6'], dtype=np.float32)
        y = np.array(Z['y_true'][:, :], dtype=np.float32)
        #y = np.array(Z['y'][:1, :], dtype=np.float32)
        weights = Z['w'] # those are the weight of the output layer
        ...
        # reshape to one dimension
        #max_1 = (x.shape[1] // self.mini_batch_size) * self.mini_batch_size
        #lim = (max_1 // self.batch_size) * self.batch_size
        #x = x[:, :lim]
        #y = y[:, :lim]
        #rep = x.shape[1]
        # loading the conditioning values
        z = np.array(Z['z'], dtype=np.float32)
        #z = np.repeat(z, rep, axis=1)
    ...
    def on_epoch_end(self):
        # create/reset the vector containing the indices of the batches
        self.indices = np.arange(0, self.x.shape[0])
    ...
    def __getitem__(self, idx):
        ...
        # fill the batches
        X = np.array(self.x[indices]).reshape(self.batch_size, self.mini_batch_size, 1)
        YH = np.array(self.yh[indices2])
        Y = np.array(self.y[indices]).reshape(self.batch_size, self.mini_batch_size, 1)
        Z = np.array(self.z[indices]).reshape(self.batch_size, self.mini_batch_size, 1)

        return [Z, X], [YH, Y]

class DataGeneratorCondPicklesTest(Sequence):
