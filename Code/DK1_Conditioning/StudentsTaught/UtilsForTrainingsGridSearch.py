...
def plotResult(pred, inp, tar, z, model_save_dir, save_folder, fs, filename):
    ...
    for i in range(0, l - 1, 1):
        ...
        ax.set_title('Drive: ' + str(z[i * fs * seconds]))
...
def plotTraining(loss_training, loss_val, model_save_dir, save_folder, name):
    ...
    # , bbox_to_anchor=(0.5, -0.15), fancybox=True, shadow=False, ncol=2)
...
def predictWaves(pred, x_test, y_test, z, model_save_dir, save_folder, fs, filename):
    ...
    pred_dir = os.path.normpath(os.path.join(
        model_save_dir, save_folder, 'WavPredictions', pred_name))
    inp_dir = os.path.normpath(os.path.join(
        model_save_dir, save_folder, 'WavPredictions', inp_name))
    tar_dir = os.path.normpath(os.path.join(
        model_save_dir, save_folder, 'WavPredictions', tar_name))
...
def checkpoints(model_save_dir, save_folder):
    ...
    ckpt_path_latest = os.path.normpath(os.path.join(
        model_save_dir, save_folder, 'Checkpoints', 'latest', 'latest.ckpt'))
    ckpt_dir = os.path.normpath(os.path.join(
        model_save_dir, save_folder, 'Checkpoints', 'best'))
    ckpt_dir_latest = os.path.normpath(os.path.join(
        model_save_dir, save_folder, 'Checkpoints', 'latest'))
    ...
    ckpt_callback_latest = tf.keras.callbacks.ModelCheckpoint(
        filepath=ckpt_path_latest, monitor='val_loss', mode='min', save_best_only=False, save_weights_only=True, verbose=1)
