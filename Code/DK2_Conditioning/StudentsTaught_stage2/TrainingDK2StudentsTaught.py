...
def trainDK2(**kwargs):
    ...
    if not inference:
        ...
        weight_gen = DataGeneratorPicklesTrain(data_dir, 'DK_Teacher_' + dataset_train + '_train.pickle',
                                             input_size=input_dim, conditioning_size=conditioning_size,
                                             batch_size=batch_size)

        if model.layers[-1].name == 'OutLayer':
            model.layers[-1].set_weights(weight_gen.weights)
        elif model.layers[-2].name == 'OutLayer':
            model.layers[-2].set_weights(weight_gen.weights)
