...
def MFCC(y_true, y_pred, sr):
    ...
    for i in range(len(m)):
        ...
        Y_true = tf.signal.stft(
            y_true, frame_length=m[i], frame_step=m[i]//4,  fft_length=m[i], pad_end=False)
        Y_pred = tf.signal.stft(
            y_pred, frame_length=m[i], frame_step=m[i]//4,  fft_length=m[i], pad_end=False)
        ...
        mel_spectrograms_pred = tf.tensordot(
            Y_pred, linear_to_mel_weight_matrix, 1)
        mel_spectrograms_pred.set_shape(
            Y_pred.shape[:-1].concatenate(linear_to_mel_weight_matrix.shape[-1:]))

        mel_spectrograms_tar = tf.tensordot(
            Y_true, linear_to_mel_weight_matrix, 1)
        mel_spectrograms_tar.set_shape(
            Y_true.shape[:-1].concatenate(linear_to_mel_weight_matrix.shape[-1:]))

        # Compute a stabilized log to get log-magnitude mel-scale spectrograms.
        log_mel_spectrograms_pred = tf.math.log(mel_spectrograms_pred + 1e-6)

        # Compute MFCCs from log_mel_spectrograms and take the first 13.
        mfccs_pred = tf.signal.mfccs_from_log_mel_spectrograms(
            log_mel_spectrograms_pred)

        log_mel_spectrograms_tar = tf.math.log(mel_spectrograms_tar + 1e-6)

        # Compute MFCCs from log_mel_spectrograms and take the first 13.
        mfccs_tar = tf.signal.mfccs_from_log_mel_spectrograms(
            log_mel_spectrograms_tar)

        loss += tf.norm((mfccs_tar - mfccs_pred), ord=1) / \
            (tf.norm(mfccs_tar, ord=1))
...
def STFT_loss(y_true, y_pred):  # auraloss multi-STFT
    m = [256, 512, 1024]
    y_true_ = tf.reshape(y_true, [-1])
    y_pred_ = tf.reshape(y_pred, [-1])
    ...
    for i in range(len(m)):
        ...
        Y_true = K.abs(tf.signal.stft(
            y_true, fft_length=m[i], frame_length=m[i], frame_step=m[i] // 4, pad_end=False))
        Y_pred = K.abs(tf.signal.stft(
            y_pred, fft_length=m[i], frame_length=m[i], frame_step=m[i] // 4, pad_end=False))

        Y_true = tf.pow(K.abs(Y_true), 2)
        Y_pred = tf.pow(K.abs(Y_pred), 2)

        loss += (tf.norm((Y_true - Y_pred), ord=1) /
                 (tf.norm(Y_true, ord=1) + 1e-9))
...
def SF(y_true, y_pred):  # Spectral difference
    ...
    Y_true = K.abs(tf.signal.stft(y_true, fft_length=512,
                   frame_length=window, frame_step=window, pad_end=True))
    Y_pred = K.abs(tf.signal.stft(y_pred, fft_length=512,
                   frame_length=window, frame_step=window, pad_end=True))

    loss = 0.
    for n in range(1, tf.shape(Y_true)[0]):
        sf_true = tf.reduce_sum(tf.divide(K.abs(
            Y_true[n]) - K.abs(Y_true[n - 1] + K.abs(K.abs(Y_true[n]) - K.abs(Y_true[n - 1]))), 2) ** 2)
        sf_pred = tf.reduce_sum(tf.divide(K.abs(
            Y_pred[n]) - K.abs(Y_pred[n - 1] + K.abs(K.abs(Y_pred[n]) - K.abs(Y_pred[n - 1]))), 2) ** 2)
