...
Put all datasets in the "src./datasets" folder as such:
```
./src
├── code
├── datasets
...
A selection of pre-trained teacher and student models can be found under the "./src/models" folder. Although both conditioned and unconditioned versions are listed, conditioned models currently only exist only for DrDrive.
...
Therefore, the naming convetion is "architechure_device_numb-units-in-the-second-LSTM-layer". The folder structure:
```
./src
├── code
├── datasets
...
Next, choose whether to train a teacher, distilled student or non-distilled student network. Recommended order of operatations:
...
└── models
``` 
Every subfolder inside ".src/code/" contains its own starter script for running the code from a terminal with docs on avaliable arguments.
